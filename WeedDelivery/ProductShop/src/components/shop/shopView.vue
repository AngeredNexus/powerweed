  ...
  <div class="min-h-screen flex flex-col">
    ...
    <header class="flex justify-between items-center p-4 bg-gray-800 text-white shadow-md">
      ...
      <div class="flex items-center space-x-4">
        ...
        <button
            @click="toggleCart"
            class="bg-gray-700 p-2 rounded-full hover:bg-gray-600 transition duration-300"
        >
          🛒
        </button>
      </div>
    ...
    <main class="flex-1 p-4 overflow-hidden">
      <transition name="fade" mode="out-in">
        <div :key="currentViewComponent">
          ...
          <ProductList v-if="currentView === 'ProductList' && isLoaded"
                       :products.sync="products"
                       @select-product="selectProduct"/>
          
          <ProductView v-if="currentView === 'ProductView'"
                       :product.sync="selectedProduct"
                       @update-quantity="updateQuantity"
                       @backView="selectedProductExit"/>
        </div>
      </transition>
    </main>
    ...
    <transition name="fade">
      <Cart v-if="isCartVisible && isLoaded"
            :cartItems.sync="cartItems"
            :cartPrice.sync="isHaveDiscount ? discountedPrice : cartPrice"
            @update-quantity="updateQuantity"
